<?php
// laravel_setup_debug.php - Complete Laravel setup script with detailed debugging for cPanel deployment
...
function runArtisanCommand($command, $params = []) {
    ...
    if ($kernel === null) {
        ...
        $vendorPath = __DIR__ . DS . 'vendor' . DS . 'autoload.php';
        if (!file_exists($vendorPath)) {
            throw new Exception('Laravel vendor autoload not found at: ' . $vendorPath . '. Please make sure you uploaded vendor folder.');
        }
        
        logMessage("✅ Found vendor/autoload.php");
        
        // Include Laravel components
        require_once $vendorPath;
        
        // Check if Laravel app files exist
        $bootstrapPath = __DIR__ . DS . 'bootstrap' . DS . 'app.php';
        if (!file_exists($bootstrapPath)) {
            throw new Exception('Laravel bootstrap/app.php not found at: ' . $bootstrapPath . '. Please ensure complete Laravel application is uploaded.');
        }
        
        logMessage("✅ Found bootstrap/app.php");
        ...
        logMessage("✅ Laravel application kernel initialized");
    }
...
try {
    ...
    logMessage("📁 Current directory: " . __DIR__);
    logMessage("🌐 Base URL would be: " . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'unknown') . dirname($_SERVER['SCRIPT_NAME']));
    
    // Check if necessary Laravel directories exist
    $requiredDirs = ['app', 'bootstrap', 'config', 'database', 'public', 'resources', 'storage', 'vendor'];
    foreach ($requiredDirs as $dir) {
        $path = __DIR__ . DS . $dir;
        if (!is_dir($path)) {
            throw new Exception("Required directory '$dir' not found at: $path");
        } else {
            logMessage("✅ Found directory: $dir");
        }
    }
    ...
    foreach ($requiredFiles as $file) {
        $path = __DIR__ . DS . $file;
        if (!file_exists($path)) {
            logMessage("⚠️  Warning: Required file '$file' not found at: $path (this might be OK in some cases)");
        } else {
